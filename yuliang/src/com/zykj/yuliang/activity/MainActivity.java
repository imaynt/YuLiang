...
public class MainActivity extends BaseActivity implements View.OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.ll_youhuiquan:
			 ...
			 * 跳转兑吧
			 */
			if (StringUtil.isEmpty(BaseApp.getModel().getMoney())) {
				Tools.toast(MainActivity.this, "请先做新手任务和完善个人资料");
			} else {
				...
				if (str[0].equals("0")) {
					Tools.toast(MainActivity.this, "由于您的金额为0，请先做任务获得金额");
				} else {
					HttpUtils.getLoginUrl(new HttpErrorHandler() {
						@Override
						public void onRecevieSuccess(JSONObject json) {
							String url = json.getJSONObject(UrlContants.jsonData).getString("url");
							Intent intent = new Intent().setClass(MainActivity.this, CreditActivity.class);
		...
		case R.id.ll_shengqian:// 省钱
			...
			// AppSecret);//实例化TopClient类
			// UserSellerGetRequest req = new
			// UserSellerGetRequest();//实例化具体API对应的Request类
	...
	private void requestData() {
		...
		HttpUtils.getPoints(new HttpErrorHandler() {
			...
			@Override
			public void onRecevieSuccess(JSONObject json) {
				account = json.getJSONObject(UrlContants.jsonData).getString("account");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {// 返回按钮
			...
			builder.setTitle("提示").setMessage("您确定退出当前应用").setNegativeButton("取消", new OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
				}
